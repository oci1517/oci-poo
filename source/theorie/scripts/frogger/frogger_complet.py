...
# Dans cette étape on implémente la gestion du temps limite
...
class FroggerGame(object):
    ...
    def __init__(self):
        ...
        # variable d'instance
        self.nb_succes = 0
        # temps à disposition pour terminer la traversée en [s]        
        self.max_temps = 15
    ...
    def manager(self):
        while not isDisposed() and not self.is_game_over:
            ...
            infos = '#Vies : {vies} // #Succès : {success} // #Points : {points} // Temps restant : {temps}'
            infos = infos.format(vies=self.frog.vies,
                                 success=self.nb_succes,
                                 points=self.frog.points,
                                 temps=self.frog.temps_restant)
            setTitle(infos)
            # pour ne pas surcharger le processeur et lui laisser du temps
            # pour faire autre chose que de mettre à jour la barre de titre
            delay(100)
...
class Frog(Actor):
    ...
    def collide(self, actor1, actor2):
        ...
        play()
        # on remet la grenouille à sa position de départ
        self.reset()
        # lors d'une collision, la grenouille perd une vie et 5 points
        self.vies -= 1
        self.points -= 5    
        return 0
